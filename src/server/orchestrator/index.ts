...
export class Orchestrator {
  ...
  private branchPreambleLines(runId: number, branchName: string | null): string[] {
    const mirror = `claude/run-${runId}`;
    const isAutoGenerated = !branchName || branchName === mirror;
    if (isAutoGenerated) {
      return [
    ...
    return [
      `You are working on branch \`${branchName}\`. Make all commits here.`,
  ...
  async launch(runId: number): Promise<void> {
    ...
    let branchNameWatcher: BranchNameWatcher | null = null;
    ...
    try {
      ...
      if (branchNameWatcher) await branchNameWatcher.stop();
  ...
  async resume(runId: number): Promise<void> {
    ...
    try {
      ...
      titleWatcher.start();
      const branchNameWatcher = new BranchNameWatcher({
        path: `${this.stateDirFor(runId)}/branch-name`,
        pollMs: 1000,
        onBranchName: (name) => { this.deps.runs.setBranchName(runId, name); },
        onError: () => { /* swallow — best effort */ },
