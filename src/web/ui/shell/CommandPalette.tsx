    ...
    >
      ...
      >
        <Command.Input
          ...
          className="w-full px-4 py-3 bg-transparent font-mono text-[14px] text-text placeholder:text-text-faint border-b border-border outline-none"
        />
        <Command.List className="max-h-80 overflow-auto py-1">
          <Command.Empty className="px-4 py-3 text-[13px] text-text-faint">No results.</Command.Empty>
          {groups.map((g) => {
            const rows = items[g.id] ?? [];
            if (rows.length === 0) return null;
            return (
              <Command.Group key={g.id} heading={g.label} className="[&_[cmdk-group-heading]]:px-4 [&_[cmdk-group-heading]]:py-1.5 [&_[cmdk-group-heading]]:text-[11px] [&_[cmdk-group-heading]]:font-semibold [&_[cmdk-group-heading]]:uppercase [&_[cmdk-group-heading]]:tracking-[0.08em] [&_[cmdk-group-heading]]:text-text-faint">
                {rows.map((it) => (
                  <Command.Item
                    key={it.id}
                    value={`${it.label} ${it.keywords?.join(' ') ?? ''}`}
                    onSelect={() => { it.onSelect(); onClose(); }}
                    className="flex items-center gap-2 px-4 py-1.5 text-[13px] text-text-dim aria-selected:bg-accent-subtle aria-selected:text-accent-strong cursor-pointer"
                  >
                    <span className="flex-1 min-w-0 truncate">{it.label}</span>
                    {it.hint && <span className="font-mono text-[12px] text-text-faint">{it.hint}</span>}
                  </Command.Item>
                ))}
              </Command.Group>
            );
