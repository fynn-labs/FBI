      ...
      {github && run.state === 'succeeded' && (
        <Group label="GitHub">
          {!github.github_available ? (
            <p className="text-[13px] text-text-faint">no gh / non-github</p>
          ) : github.pr ? (
            <>
              <a href={github.pr.url} target="_blank" rel="noreferrer" className="block text-[13px] text-accent underline">
                PR #{github.pr.number}
              </a>
              <p className="text-[13px] text-text-dim truncate">{github.pr.title}</p>
              {github.checks && (
            ...
            <button
              onClick={onCreatePr}
              disabled={creatingPr}
              className="text-[13px] text-accent hover:text-accent-strong disabled:opacity-50"
            >
